...
const Routine = () => {
  ...
  return (
    ...
    <Container>
      ...
      />
      <RoutineEditModal note={targetNote} open={openEdit} handleClose={handleNoteEditClose} setNotes={setNotes} />
      <RoutineCreateModal open={openCreate} handleClose={handleNoteCreateClose} setNotes={setNotes} />
      
      <Typography fontSize={30} color={'black'} marginTop={'50px'}>Set Routine</Typography>
        <Box sx={{color: 'white' , width: '100px', marginTop: '10px'}}>
          <CustomButton text="Add +" handle={handleNoteCreateOpen} fontSize={18} />
        </Box>
      <Typography fontSize={30} color={'black'} marginTop={'50px'} marginBottom={'10px'}>My Routine</Typography>
      {user ? (
        notes.length === 0 ? (
          ...
          <Grid container spacing={2}>
            {notes.map((note, index) => (
              <Grid item xs={4} key={index}>
                <RoutineCard title={note.title} date={note.updatedAt} handleClick={() => handleTargetNoteChange(note)} />
              </Grid>
            ))}
          </Grid>
        )
      ) : (
        <Typography textAlign="center" fontSize={18} color="black" fontWeight={300} marginTop={8}>
          You must login to to set a routine.
        </Typography>
      )}
